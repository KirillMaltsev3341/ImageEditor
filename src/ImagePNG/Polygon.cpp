...
void getIntersections(std::vector<std::pair<int, int>>& intersections, int y, std::vector<ie::Coord>& vertices)
{
    for (int i = 0; i < vertices.size(); i++) {
        int next = (i + 1) % vertices.size();
...
void ie::ImagePNG::fillPolygon(std::vector<Coord>& vertices, Color& fill_color)
{
    int y_min = INT_MAX;
    int y_max = INT_MIN;
    for (int i = 0; i < vertices.size(); i++) {
